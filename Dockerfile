...
RUN apt-get update \
    ...
    && apt-get update \
    && apt-get -y --no-install-recommends install php8.4 php8.4-fpm php8.4-cli $(cat /tmp/php-packages.txt | sed 's/php\([0-9.]*\)-/php8.4-/g') \
...
RUN service php8.4-fpm stop || true

# 既存の自動起動プロセスを確実に殺し、ランタイム用のディレクトリを作成
RUN mkdir -p /run/php && service php8.4-fpm stop || true
